...
# taken from https://github.com/fzuerkeraguilar/alpha-beta-deepZ/blob/main/abZono/zonotope.py
...
class Zonotope:
    ...
    def __getitem__(self, subscript):
        return Zonotope(self.center[subscript], self.generators[:, subscript])
    ...
    def sample_boundary(self, D=None, n_samples=100):
        """
        Samples points form the boundary of the zonotope. (exclusively samples vertices)

        This is done by randomly sampling directions d in which to maximize
        d^T x for x in zonotope z.
    ...
    @staticmethod
    def from_bounds(l, u, shape: torch.Size, dtype: torch.dtype):
        #center = 0.5 * (l + u)
        #generators = 0.5 * (u - l)
